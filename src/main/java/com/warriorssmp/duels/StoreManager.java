...
public class StoreManager {
     ...
     *  situation warrants instead. Catches Exception broadly (not just
     *  IOException/InvalidConfigurationException) because store rewards
     *  are stored via Bukkit's own ItemStack serialization, which - like
     *  the real, confirmed Location crash this same pattern caused in
     *  ArenaManager - deserializes EAGERLY as part of the raw
     *  yaml.load(...) parse itself. An ItemStack referencing a Material
     *  that's been renamed/removed in a newer Minecraft version, or any
     *  other corrupted item data, could throw an unchecked exception
     *  from deep inside that parse - this plugin's entire onEnable()
     *  must never be able to crash over one bad store item. */
    private void load() {
